...
| Feature | Description |
|---|---|
| 🚀 **One-Click Download** | Launches `animepahe-dl` globally via PowerShell |
...
| 📦 **Auto Dependency Install** | Reads `Requirements.txt` and installs missing packages globally on startup |
| 🖥️ **Windowed Scripts** | Opens PowerShell scripts in a new console window for live feedback |
| 🌐 **No Virtual Environment** | Everything runs globally — just Python + pip, nothing else needed |
...
├── animepahe.ps1            # PowerShell script to run animepahe-dl globally
├── fixing.ps1               # PowerShell script to batch-fix MP4 audio via ffmpeg
├── Requirements.txt         # Python dependencies (auto-installed globally on launch)
│
...
- **pip** available globally (comes with Python)
- **Windows OS** (PowerShell required)
- **ffmpeg** installed and added to your system `PATH` (for the audio fix feature)

> ✅ No virtual environment needed. Everything installs and runs globally.
...
> 💡 This step is completely optional — `launcher.py` auto-installs everything on first launch.
...
> 💡 On first run, any missing packages from `Requirements.txt` are automatically installed globally. Just run and go.
...
No configuration needed. The script checks if `animepahe-dl` is available globally and installs it automatically if missing, then runs it directly.
...
│  (Main Window)  │  Auto-installs Requirements.txt globally
└────────┬────────┘
         │
    ┌────┴────┐
    │         │
...
1. **Launch** `launcher.py` — missing packages are auto-installed globally, then the main GUI opens.
2. **Download Anime** — opens a new PowerShell console, checks/installs `animepahe-dl` globally if needed, and runs the downloader interactively.
...
| `animepahe-dl` | Downloading anime from AnimePahe |
| `ffmpeg` *(system)* | Re-encoding MP4 audio streams to AAC format |

> `pillow` and `animepahe-dl` are auto-installed globally on launch. `ffmpeg` must be installed manually and added to your system `PATH`.
